...
export const WooSeokBotDetails: ProjectSection[] = [
  ...
  {
    title: "핸들링 고도화",
    images: [{ src: "/wooseokbot_code.webp", alt: "카카오 지도 정보" }],
    items: [
      {
        title: "질문 의도 파악과 예외 처리 강화를 위한 AI 응답 핸들링 고도화",
        content: `단순히 사용자의 질문에 답하는 구조에서 나아가, 문맥 기반의 판단과 예외 상황에 대한 대응까지 설계한 것이 이번 챗봇 고도화의 핵심입니다.

- 특정 프로젝트에 대한 질문인지 판단하기 위해 slug 기반 매칭 로직을 우선 수행하고, 명시되지 않은 경우 GPT를 활용해 유사도 기반으로 가장 적절한 항목을 추출합니다.
- OpenAI 응답이 지연되거나 실패하는 경우를 대비해, 사용자에게 명확하고 직관적인 피드백을 제공하도록 에러 메시지와 로딩 상태 UI를 강화했습니다.
- 사용자가 직전 질문을 되묻는 경우를 인식하여, 대화형 인터페이스의 자연스러운 흐름을 유지할 수 있도록 설계했습니다.
- 챗봇은 항상 '박우석 지원자'의 시점에서 응답하도록 프롬프트를 엄격하게 제어하며, 시스템 내부 표현(ex. DB, 입력된 정보 등)은 사용자에게 노출되지 않도록 주의했습니다.
- 유저의 질문이 단일 프로젝트에 대한 내용인지, 일반 이력 정보인지 구분하기 위해 fallback 전략과 다단계 매칭 구조를 구성했습니다.

결과적으로, 단순한 질문-응답 챗봇을 넘어, 맥락 이해, 에러 대응, 지능형 타이틀 매칭, 사용자 경험 향상까지 고려된 고도화된 AI 응답 처리 구조를 완성했습니다.`,
      },
    ],
  },
  {
    ...
    items: [
      {
        title: "응답 대기 시 피드백 제공",
        content:
          "답변 생성 중에는 점(.)이 순차적으로 깜빡이는 애니메이션을 통해 로딩 상황을 명확히 전달합니다.",
      },
      {
        title: "에러/예외 상황 처리 코드 리팩터링",
        content:
          "챗봇의 답변 생성 과정에서 발생할 수 있는 예외와 오류 상황을 안정적으로 처리하기 위해 코드 구조를 리팩터링하고, 예외 처리 로직을 강화했습니다.",
      },
      {
        title: "서버 응답 실패 시 안내 메시지",
        content:
          "서버가 꺼져있거나 응답이 없을 때, '박우석 지원자가 잠시 눈을 붙이고 있어요. 깨우는 중입니다...' 등과 같은 안내 메시지를 노출하여, 사용자가 이유를 명확하게 알 수 있도록 했습니다.",
      },
    ],
  },
  {
    ...
    items: [
      {
        title: "크롬 Lighthouse 기반 성능/접근성 측정",
        content:
          "Jest 등 자동화 테스트 도구는 별도 도입하지 않았으며, 개발 단계 및 배포 전 단계에서 Chrome Lighthouse를 활용해 주요 페이지의 성능, 접근성, SEO 지표를 측정·개선하였습니다.",
      },
    ],
  },
  {
